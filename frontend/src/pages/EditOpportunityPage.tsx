...
import { CalendarDays, Link as LinkIcon, AlignLeft, CheckSquare, Globe, Upload, MapPin, Building2, ShieldCheck, ChevronLeft } from 'lucide-react';
...
const EditOpportunityPage: React.FC = () => {
      ...
      .then(data => {
        if (data) {
          setForm({
            ...
            officialUrl: data.official_website || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Process tags
      const tagNames = form.tags.split(',').map((t: string) => t.trim()).filter((t: string) => t.length > 0);
      let tag_ids: number[] = [];
    ...
    <div className="bg-[#F8F7F4] min-h-screen flex flex-col">
      ...
      <div className="bg-[#1B5442] pt-[72px]">
        <div className="container-max py-12">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/60 hover:text-white text-sm font-medium mb-6 transition-colors">
            <ChevronLeft size={16} /> Back
          </button>
          <span className="badge bg-white/20 text-white/90 mb-4">Edit</span>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-3" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            Edit Opportunity
          </h1>
          <p className="text-white/70 max-w-xl">
            Modify the details of this opportunity. Changes will be reflected instantly.
          </p>
        </div>
      </div>
      ...
      <main className="container-max py-10 flex-grow max-w-3xl">
        {loading ? (
          ...
          <div className="flex justify-end gap-3 pb-4">
            <button type="button" onClick={() => navigate(-1)} className="btn-ghost px-6 py-3">Cancel</button>
            <button type="submit" disabled={submitting} className="btn-primary px-8 py-3 gap-2 disabled:opacity-70">
              <Upload size={16} /> {submitting ? 'Saving...' : 'Save Changes'}
            </button>
          </div>
        </form>
        </>
